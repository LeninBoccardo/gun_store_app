...
class AccessoryScreen extends StatefulWidget {
  @override
  _AccessoryScreenState createState() => _AccessoryScreenState();
}

class _AccessoryScreenState extends State<AccessoryScreen> {
  final db = Firestore.instance;
  final String collection = "accessory";

  List<Accessory> list = List();
  ...
  void initState() {
    ...
    listen = db.collection(collection).snapshots().listen((res) {
      setState(() {
        list = res.documents
            .map((doc) => Accessory.fromMap(doc.data, doc.documentID))
            .toList();
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Seção de acessórios"),
        centerTitle: true,
                ...
                default:
                  ...
                  return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        onTap: () {
                          Navigator.pushNamed(context, "/aRegister",
